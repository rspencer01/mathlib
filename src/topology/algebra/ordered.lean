...
-/
def partial_order.topology : topological_space α :=
generate_from {s : set α | ∃ (a : α), s = {b : α | a < b} ∨ s = {b : α | b < a}}

local attribute [instance] partial_order.topology

/-- Shows that `partial_order.topology` indeed produces an orderable topology. -/
lemma partial_order.order_topology : orderable_topology α := ⟨rfl⟩
...
lemma decidable_linear_ordered_comm_group.order_topology.tendsto_nhds {β : Type*}
